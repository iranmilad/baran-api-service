...
class ProcessProductChanges implements ShouldQueue
{
    ...
    public function handle()
    {
        ...
        try {
            ...
            $implicitInserts = []; // محصولاتی که در update درج می‌شوند
            $failedInserts = []; // محصولاتی که درج ناموفق است
            ...
            $processedInserts = $this->createNewProducts($parentProducts, $childProducts, $productsToCreate, $failedInserts);
            ...
            if (!empty($implicitInserts)) {
                RetryFailedProductInserts::dispatch($implicitInserts, $this->license_id);
            }
        ...
        foreach ($this->changes as $change) {
            ...
            if ($productData['is_variant'] === true) {
                if (empty($productData['parent_id']) || trim($productData['parent_id']) === '') {
                    ...
                    $this->processParentProduct($productData, $barcode, $changeType, $existingProducts, $parentProducts, $productsToUpdate, $updateIds, $variantsToDelete, $implicitInserts);
                } else {
                    ...
                    $this->processChildProduct($productData, $barcode, $changeType, $existingProducts, $childProducts, $productsToUpdate, $updateIds, $implicitInserts);
                }
            } else {
                // محصول معمولی (is_variant=false)
                $this->processRegularProduct($productData, $barcode, $changeType, $existingProducts, $productsToCreate, $productsToUpdate, $updateIds, $variantsToDelete, $implicitInserts);
            }
     ...
     * @return void
     */
    protected function processParentProduct($productData, $barcode, $changeType, $existingProducts, &$parentProducts, &$productsToUpdate, &$updateIds, &$variantsToDelete, &$implicitInserts)
    {
        if ($changeType === 'insert') {
            ...
            if ($existingProduct) {
                ...
                // به عنوان درج ضمنی ذخیره می‌شود
                Log::info("درخواست update برای محصول مادر {$barcode} - محصول وجود ندارد، به درج ضمنی اضافه شد");
                $implicitInserts[] = $productData;
            }
        }
    }
     ...
     * @return void
     */
    protected function processChildProduct($productData, $barcode, $changeType, $existingProducts, &$childProducts, &$productsToUpdate, &$updateIds, &$implicitInserts)
    {
        if ($changeType === 'insert') {
            ...
            if ($existingProduct) {
                ...
                // به عنوان درج ضمنی ذخیره می‌شود
                Log::info("درخواست update برای محصول متغیر {$barcode} - محصول وجود ندارد، به درج ضمنی اضافه شد");
                $implicitInserts[] = $productData;
            }
        }
    }
     ...
     * @return void
     */
    protected function processRegularProduct($productData, $barcode, $changeType, $existingProducts, &$productsToCreate, &$productsToUpdate, &$updateIds, &$variantsToDelete, &$implicitInserts)
    {
        if ($changeType === 'insert') {
            ...
            if ($existingProduct) {
                ...
                // به عنوان درج ضمنی ذخیره می‌شود
                Log::info("درخواست update برای محصول {$barcode} - محصول وجود ندارد، به درج ضمنی اضافه شد");
                $implicitInserts[] = $productData;
            }
        }
    }
     ...
     * @param array $childProducts
     * @param array $productsToCreate
     * @param array &$failedInserts
     * @return array محصولات ایجاد شده
     */
    protected function createNewProducts($parentProducts, $childProducts, $productsToCreate, &$failedInserts)
    {
        ...
        foreach ($allProducts as $product) {
            try {
                ...
                // درج محصول ناموفق برای retry بعدی
                // این احتمالا به دلیل عدم وجود parent_id است
                $failedInserts[] = $product;
            }
        }
